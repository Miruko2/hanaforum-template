...
import { navigateWithFlip, PAGE_RING } from "@/lib/view-transition-nav"
...
// 全局左右轻扫切页（仅触屏）：首页 ⇄ 弹幕墙 ⇄ 音乐 ⇄ 个人中心。
// 手指左滑 = 去环上右侧的下一页，配合 3D 翻页转场。
export default function PageSwipe() {
  const router = useRouter()
  const pathname = usePathname()
  const { user } = useSimpleAuth()

  // 监听器只绑一次，路径/登录态/router 通过 ref 取最新值
  ...
  useEffect(() => {
    ...
    const onTouchStart = (e: TouchEvent) => {
      ...
      // music 页整面画布是横向拖拽交互（拖卡片墙），切页让位、走导航栏
      if (pathRef.current === "/music") return
    ...
    const onTouchEnd = (e: TouchEvent) => {
      ...
      const idx = ring.indexOf(pathRef.current)
      if (idx === -1) return
      // 首尾循环：首页再往回滑直接转到环尾（个人中心），环尾再往前滑回首页
      const nextIdx = (idx + (dx < 0 ? 1 : -1) + ring.length) % ring.length

      navigateWithFlip(routerRef.current, ring[nextIdx], dx < 0 ? "next" : "prev")
    }
