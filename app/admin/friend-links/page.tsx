...
// 友链管理后台（/admin/friend-links）。独立子页，刻意不并进 app/admin/page.tsx，
// 避免与并行开发的审核面板改动撞同一文件。鉴权：前端 useSimpleAuth 门禁 + 后端 API requireAdmin 双层。
import { useCallback, useEffect, useState } from "react"
import { useRouter } from "next/navigation"
import Link from "next/link"
import {
  Check, X, Trash2, Eye, EyeOff, ChevronUp, ChevronDown, Plus, Pencil,
  Loader2, ExternalLink, Inbox, ArrowLeft, ShieldAlert,
} from "lucide-react"
...
export default function FriendLinksAdminPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [r1, r2] = await Promise.all([
        fetch("/api/admin/friend-links", { headers: h }),
        fetch("/api/admin/friend-links/submissions?status=pending", { headers: h }),
      ])
  ...
  async function actSubmission(id: string, action: "approve" | "reject" | "spam") {
    ...
    try {
      const h = await authHeaders()
      const res = await fetch("/api/admin/friend-links/submissions", {
        method: "PATCH",
        headers: h,
  ...
  async function deleteLink(link: FriendLinkRow) {
    ...
    try {
      const h = await authHeaders()
      const res = await fetch(`/api/admin/friend-links?id=${encodeURIComponent(link.id)}`, {
        method: "DELETE",
        headers: h,
      })
      if (!res.ok) {
    ...
    <main className="min-h-screen text-white">
      <div className="container mx-auto max-w-3xl px-4 pt-24 pb-16 space-y-8">
        ...
        <div className="flex items-center justify-between gap-3">
          <div>
            <h1 className="text-2xl font-bold sm:text-3xl">友链管理</h1>
            <p className="mt-1 text-sm text-white/50">审核申请、手动增删改友链，改动即时同步到 /links</p>
          </div>
          <div className="flex items-center gap-2">
            <Link
              href="/links"
              target="_blank"
              className="inline-flex items-center gap-1.5 rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-sm text-white/70 transition hover:text-lime-300"
            >
              看 /links <ExternalLink className="h-3.5 w-3.5" />
            </Link>
            <Link
              href="/admin"
              className="inline-flex items-center gap-1.5 rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-sm text-white/70 transition hover:text-white"
            >
              <ArrowLeft className="h-3.5 w-3.5" /> 管理面板
            </Link>
          </div>
        </div>
